...
saveGraph("output/censor-from-above.pdf")
...
ggplot(
    ...
    scale_y_continuous(
        ...
        labels=trans_format(function(x) 2^x, math_format(.x))) +
    facet_wrap(~visandsign)
saveGraph("output/censor-from-below.pdf")
```
...
openGraph(7.5,4)
ggplot(
        filter(df, visandsign %in% c("linepositive","parallelCoordinatespositive")),
        aes(x=r, 
            y=log2(jnd),
    )) + 
    ...
    geom_hline(yintercept=log2(.45), lty="dashed") + 
#    stat_function(fun=function(x) log2(1 - x), lty="dashed", color="black") +
    geom_line(data=filter(model_fits, visandsign %in% c("linepositive","parallelCoordinatespositive")), mapping=aes(y=prediction/log(2), color=model)) + 
    annotation_logticks(sides="l") +
    scale_y_continuous(
        limits=c(-6, -0.2),
        labels=trans_format(function(x) 2^x, math_format(.x))) +
