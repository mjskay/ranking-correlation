...
```{r linear_vs_loglinear_AIC_3, eval=FALSE}
AIC(m.linear, m.linear_v, m.loglinear)
```

```{r linear_vs_loglinear_AIC_4, echo=FALSE, results='asis'}
pander(AIC(m.linear, m.loglinear))
```
...
We can compare against a censored linear model:

```{r censored_linear}
m.censored_linear = gamlss(Surv(censored_jnd, not_censored) ~ r * visandsign * approach_value,
    sigma.formula = ~ visandsign,
    data=df,
    family=cens(NO)
    )
plot(m.censored_linear)
AIC(m.censored,m.censored_linear)
```
