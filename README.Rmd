...
m.censored2 = gamlss(Surv(log_censored_jnd, not_censored) ~ r * visandsign * approach,
    sigma.formula = ~ visandsign,
    data=df,
    family=cens(NO)
    )
...
m.censoredt2 = gamlss(Surv(log_censored_jnd, not_censored) ~ r * visandsign * approach,
    sigma.formula = ~ visandsign,
    nu.formula = ~ visandsign,
    data=df,
    family=cens(TF)
    )

m.censored.bct = gamlss(Surv(censored_jnd, not_censored) ~ r * visandsign * approach,
    data=df,
    family=cens(BCT)
    )

m.bccg = gamlss(jnd ~ r * visandsign * approach,
    sigma.formula = ~ visandsign,
    data=filter(df, !mad_cutoff & !p_chance_cutoff),
    family=BCCG
    )

m.bct = gamlss(jnd ~ r * visandsign * approach,
    sigma.formula = ~ visandsign,
    data=filter(df, !mad_cutoff & !p_chance_cutoff),
    family=BCT
    )
