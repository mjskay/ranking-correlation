...
adjusted_weber_df = weber_df %>%
    group_by(r) %>%
    summarize(mean.jnd=mean(jnd)) %>%
    join(weber_df) %>%
    mutate(r_A = r + ifelse(approach == "above", 0.5, -0.5) * mean.jnd) 
...
#saveGraph("output/censored-example", "pdf")
```    
...
coefs = coef(m.censored)
visandsigns.1 = levels(df$visandsign)[-1]
model_fits = rbind(data.frame(
    visandsign = levels(df$visandsign)[1],
    intercept = coefs[["(Intercept)"]], 
    slope = coefs[["r"]]
), data.frame(
    visandsign = visandsigns.1,
    intercept = coefs[["(Intercept)"]] + coefs[paste0("visandsign", visandsigns.1)], 
    slope = coefs[["r"]] + coefs[paste0("r:visandsign", visandsigns.1)] 
)) %>% mutate(
    intercept = intercept / log(2),
    slope = slope / log(2)
)
...
ggplot(
    ...
    stat_function(fun=function(x) log2(1 - x), lty="dashed", color="black") + 
    stat_function(fun=function(x) log2(x), lty="dashed", color="black") +
    geom_abline(data=model_fits, mapping=aes(intercept, slope)) +
    facet_wrap(~visandsign)
```
