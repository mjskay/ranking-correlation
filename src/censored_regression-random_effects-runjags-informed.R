...
model {
	...
	for (v in 1:n_visandsigns) {
		...
		b[v,1] ~ dnorm(log(.45), 0.04)	#prior on intercept is chance
		b[v,2] ~ dnorm(0, 0.01)
		b[v,3] ~ dnorm(0, 0.5)
		b[v,4] ~ dnorm(0, 0.5)

    	#prior on variance of latent variable
		tau_inverse[v] ~ dunif(0, 25)
    	tau[v] <- 1/tau_inverse[v]

		#prior on random effects covariance matrix
		u_Sigma_inverse[v,1:2,1:2] ~ dwish(I_2, 3)
		u_Sigma[v,1:2,1:2] <- inverse(u_Sigma_inverse[v,,]) 
	}
...
parameters = c("b" , "tau", "typical_r", "typical_mu", "u_Sigma")  # The parameter(s) to be monitored.
