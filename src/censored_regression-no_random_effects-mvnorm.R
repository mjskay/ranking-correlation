...
graphics.off()
fileNameRoot="output/censored_regression_nonrandom_mvnorm" # for constructing output filenames
...
model {
	...
	for (i in 1:4) {	#length-4 vector of zeros
		b_Mean[i] <- 0
	}
	for (i in 1:4) {	#4x4 identity matrix
		for (j in 1:4) {
			b_Omega[i,j] <- equals(i, j)
		}
	}					
	for (v in 1:n_visandsigns) {
		b[v,1:4] ~ dmnorm(b_Mean[], b_Tau[v,,])
		
		#hyperprior on covariance of coefficients
    	b_Tau[v,1:4,1:4] ~ dwish(b_Omega[,], 5)
    	b_Sigma[v,1:4,1:4] <- inverse(b_Tau[v,,])
...
save.image(file=paste("output/censored_regression-no_random_effects-", 
				...
				(if (!is.null(cv_participant)) paste("-fold_", cv_participant, sep="")),
				".RData", sep=""))
#load("output/censored_regression-no_random_effects-thinned.RData")
