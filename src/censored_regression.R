...
graphics.off()
fileNameRoot="output/censored_regression_" # for constructing output filenames
...
model {
	...
	for (i in 1:n_data) {
		...
		mu[i] <- b[visandsign_number[i],1] + b[visandsign_number[i],2]*r[i] + b[visandsign_number[i],3]*approach_value[i] + 
				 u[participant_number[i],1] + u[participant_number[i],2]*r[i] 
    	y[i] ~ dlnorm(mu[i], tau[visandsign_number[i]])

		# right-censoring by the min of the two possible thresholds at this value of r
		censoring_threshold[i] <- min(jnd_threshold, jnd_r_threshold - r[i])
		y_is_censored[i] ~ dinterval(y[i], censoring_threshold[i])
  	}
    
	for (v in 1:n_visandsigns) {
		#priors for coefficents of latent models
	...
	#priors for participant random effects
	for (i in 1:2) {	#length-2 vector of zeros
		u_Mean[i] <- 0
	}
	for (p in 1:n_participants) {
		u[p,1:2] ~ dmnorm(u_Mean[], u_Tau[,])
	}
	#hyperprior on variance of participant random effects
	for (i in 1:2) {	#2x2 identity matrix
		for (j in 1:2) {
			u_Omega[i,j] <- equals(i, j)
		}
	}					
	u_Tau[1:2,1:2] ~ dwish(u_Omega[,], 3)
	u_Sigma[1:2,1:2] <- inverse(u_Tau[,])

	", if (include_predictions) "
	#predictions for ground truth
	for (i in 1:n_pred) {
		# latent variable log-linear model
		pred_u[i,1:2] ~ dmnorm(u_Mean[], u_Tau[,]) 
...
#subset data to desired plots
#df = df[df$visandsign %in% c("stackedlinenegative"),]#, "linenegative"),]
df$visandsign = factor(df$visandsign)
...
df$y = df$jnd
df$y_is_censored = df$y > jnd_threshold | df$y > jnd_r_threshold - df$r
df[df$y_is_censored,]$y = NA

#code approach as sum-to-zero
df$approach_value = ifelse(df$approach == "above", -1, 1)

#cross-valiation: drop test participant ground truth from training data
if (!is.null(cv_participant)) {
	df = df[!(df$participant_number == cv_participant	#exclude if participant is test participant 
			& df$survey_part_number < n_survey_parts	#AND survey part is ground truth
			),]	
}
...
dataList = list(
		n_data = n_data,
		y = df$y,
        y_is_censored = df$y_is_censored,
        r = df$r,
...
#get maximum likelihood estimates of model parameters from models with
#all censored data dropped
#TODO: come up with workable inits consistent with censoring
coefs = ddply(df, ~ visandsign, function(df) {
        m = with(df, gamlss(jnd ~ r + approach_value, family=LOGNO))
        c(
...
inits_list$b = NULL #first two coefficients aren't estimated well this way

#------------------------------------------------------------------------------
# RUN THE CHAINS

parameters = c("b" , "tau", "u_Sigma")  # The parameter(s) to be monitored.
...
jagsModel = jags.model( "model.txt" , data=dataList, inits=inits_list , 
...
#    stat_function(fun=function(x) log(.95 - x), lty="dashed", color="black") + 
    #    stat_smooth(method=rq, se=FALSE) +  
#    stat_smooth(method=lm) + 
    geom_point(data=df, mapping=aes(x=r, y=log(jnd), color=y_is_censored), alpha=.1) + 
    facet_wrap(~visandsign)
...
participant_sigma_sample_full = mcmcChain[, "participant_sigma"] 
...
participant_sigma_sample = as.matrix(participant_sigma_sample_full[model_number_sample == best_model])
...
if (include_predictions) {
	...
	write.csv(pred_df, file=paste("weather_rain_regression-scale_new-",
							(if (thinned_model) "thinned" else "not_thinned"),
...
#save
save.image(file=paste("output/censored_regression-", 
				(if (thinned_model) "thinned" else "not_thinned"),
				(if (!is.null(cv_participant)) paste("-fold_", cv_participant, sep="")),
				".RData", sep=""))
#load("output/censored_regression-thinned.RData")
