plotPost = function( paramSampleVec , credMass=0.95 , compVal=NULL ,
           HDItextPlace=1.0 , ROPE=NULL , yaxt=NULL , ylab=NULL ,
           xlab=NULL , cex.lab=NULL , cex=NULL , xlim=NULL , main=NULL ,
           col=NULL , border=NA , showMode=FALSE , showCurve=F , breaks=NULL , 
		   lwd.HDI=3, labelHDI=FALSE,
                     ... ) {
    ...
    if ( !showCurve ) {
	  ...
	  plot(histinfo, ylim=c(-max(histinfo$density), max(histinfo$density)),
			  xlab=xlab , yaxt=yaxt , ylab=ylab ,
			  freq=FALSE , border=border , col=col ,
			  xlim=xlim , main=main , cex=cex , cex.lab=cex.lab , ... )
	  mirroredHist = histinfo
	  mirroredHist$density = -histinfo$density
	  plot(mirroredHist, add=TRUE, 
			  freq=FALSE , border=border , col=col ,
    ...
    if ( showMode==F ) {
	    ...
	    points(meanParam, 0, pch=3, lwd=lwd.HDI, cex=cex)
    } else {
        dres = density( paramSampleVec )
        modeParam = dres$x[which.max(dres$y)]
        text( modeParam , cenTendHt ,
              bquote(mode==.(signif(modeParam,3))) , adj=c(.5,0) , cex=cex )
	 	 points(modeParam, 0, pch=3, lwd=lwd.HDI, cex=cex)
  	}
    ...
    # Display the HDI.
    lines( HDI , c(0,0) , lwd=lwd.HDI )
	if (labelHDI) {	#add text label for HDI?
