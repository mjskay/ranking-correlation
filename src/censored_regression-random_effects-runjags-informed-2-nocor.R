...
model {
	...
	for (p in 1:n_participants) {
		u[p,1] ~ dnorm(0, u_tau[participant_visandsign_number[p],1])
		u[p,2] ~ dnorm(0, u_tau[participant_visandsign_number[p],2])
	}
	...
	for (v in 1:n_visandsigns) {
		...
		u_sigma[v,1] ~ dunif(0, 3)
		u_sigma[v,2] ~ dunif(0, 3)
        u_tau[v,1] <- 1/pow(u_sigma[v,1], 2)
		u_tau[v,2] <- 1/pow(u_sigma[v,2], 2)
	}
...
if (!final_model) {
    ...
    jagsModel = autorun.jags("model.txt", data=dataList, monitor=parameters, initlist=inits_list,
        method="parallel", thin.sample=TRUE, psrf.target=2.5)    
}
