...
model {
	...
	for (p in 1:n_participants) {
		u[p,1:2] ~ dmnorm(zero_vector[], u_Sigma_inverse[participant_visandsign_number[p],,])
	}

	#priors
	for (v in 1:n_visandsigns) {
		#priors for coefficents of fixed effects
		b[v,1] ~ dnorm(log(.45), 0.11)	#prior on intercept is chance
		b[v,2] ~ dnorm(0, 0.07)
		b[v,3] ~ dnorm(0, 5)
		b[v,4] ~ dnorm(0, 5)
		...
		#prior on random effects covariance matrix
		u_sigma_b_1 ~ dunif(0, 3)
		u_sigma_b_2 ~ dunif(0, 3)
		u_rho[v] ~ dunif(-1, 1)
        u_Sigma[v,1,1] <- pow(u_sigma_b_1, 2)
        u_Sigma[v,2,2] <- pow(u_sigma_b_2, 2)
        u_Sigma[v,1,2] <- rho * u_sigma_b_1 * u_sigma_b_2
        u_Sigma[v,2,1] <- u_Sigma[1, 2]
		u_Sigma_inverse[v,1:2,1:2] <- inverse(u_Sigma[v,,]) 
	}
