...
df$y = df$jnd
df$y_is_censored = !df$not_censored
...
parameters = c("b" , "tau", "tau_u")  # The parameter(s) to be monitored.
...
#save
save.image(file=paste("output/censored_regression-random_intercept", 
				(if (thinned_model) "thinned" else "not_thinned"),
				(if (!is.null(cv_participant)) paste("-fold_", cv_participant, sep="")),
				".RData", sep=""))
